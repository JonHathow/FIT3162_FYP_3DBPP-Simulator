...
class Painter:
    ...
    def plotBoxAndItems(self,title="",alpha=0.2,write_num=False,fontsize=10):
        ...
        # plot bin 
        self._plotCube(axGlob,0, 0, 0, float(self.length), float(self.height), float(self.width),color='black',mode=1,linewidth=2,text="")
        ...
        for item in self.items:
            ...
            x,y,z = item.position
            l,h,w = item.length, item.height, item.width
            color = item.color
            text = item.name if write_num else ""

            self._plotCube(axGlob, float(x), float(y), float(z), float(l), float(h), float(w), color=color, mode=2, text=text, fontsize=fontsize, alpha=alpha)
